...
def attack_control(model, tokenizer, system_prompt, user_msg,
        ...
        for iter in range(kwargs['num_iter']):  
            ...
            if iter == 0 or (iter+1) % kwargs['test_iter'] == 0 or iter == kwargs['num_iter'] - 1:
                ...
                if iter == kwargs['num_iter'] - 1:
                    final_decoded_prompt = greedy_decode_skip_special(y_logits, tokenizer)
                    final_decoded_prompt = final_decoded_prompt[0] if isinstance(final_decoded_prompt, list) else final_decoded_prompt
    ...
    if best_y_logits is not None:
        best_decoded_prompt = greedy_decode_skip_special(best_y_logits, tokenizer)
        best_decoded_prompt = best_decoded_prompt[0] if isinstance(best_decoded_prompt, list) else best_decoded_prompt
    else:
        best_decoded_prompt = None
    
    # Return decoded prompt and the last y_logits (for multi-round optimization)
    # Can return either: final_decoded_prompt (last iteration) or best_decoded_prompt (lowest loss)
    return final_decoded_prompt if final_decoded_prompt else None, y_logits
