...
class MultimodalProductReranker:
    ...
    def _generate_adversarial_text(self, products, target_product_idx, initial_prompt_logits=None, lr=None):
        ...
        decoded_prompt, last_y_logits = attack_control(
            model=self.model,
            tokenizer=self.processor.tokenizer,
        ...
        return decoded_prompt, last_y_logits
    ...
    def _generate_adversarial_image(self, products, image_paths, target_product_idx, 
                                   global_original_tensor=None, background_mask=None):
        # Get image attack parameters from config
        ...
        mask_type = image_attack_params['mask_type']
        ...
        first_three_words = ' '.join(product_words[:3])
        ...
        if background_mask is None:
            if mask_type == 'spatial':
                ...
                print(f"\n🎭 Generating value-based mask...")
                background_mask = original_tensor.clone()
                saved_background_mask = background_mask.clone()
                print(f"✅ Value-based mask generated")
            else:
                background_mask = None
        else:
        ...
        for step in range(num_steps):
            ...
            if adversarial_tensor.grad is not None:
                ...
                if step % 10 == 0 or step + 1 == num_steps:
                    ...
                    print(f"   Step {step+1}/{num_steps} | loss={loss.item():.4f} | ‖grad‖={gn:.6f}")
            else:
                print(f"   Step {step+1}/{num_steps} | loss={loss.item():.4f} | ⚠️ No gradient!")
    ...
    def test_multimodal_ranking(self, num_products=4, mode='baseline', target_product_idx=None):
        ...
        if mode in ['attack_text', 'attack_image', 'attack_multimodal']:
            ...
            if mode == 'attack_multimodal':
                ...
                # Learning rate schedule for each round
                LR_SCHEDULE = [0.04, 0.03, 0.03, 0.02]  
                ...
                for round_idx in range(NUM_ROUNDS):
                    ...
                    decoded_prompt, last_y_logits = self._generate_adversarial_text(
                        selected_products, 
                        target_product_idx,
                ...
                if mode == 'attack_text':
                    ...
                    decoded_prompt, _ = self._generate_adversarial_text(selected_products, target_product_idx)
                    if decoded_prompt:
    ...
    def _run_single_ranking_test(self, selected_products, image_paths, target_product_idx):
        ...
        # num_tokens_to_show = min(10, len(logits_tuple))
        # if num_tokens_to_show > 0:
        #     for i in range(num_tokens_to_show):
        #         step_logits = logits_tuple[i][0]  # (vocab_size,)
