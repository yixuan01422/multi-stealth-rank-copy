...
def greedy_decode_skip_special(logits, tokenizer):
    """
    Greedy decode that skips special tokens and newlines at each position.
    For each position, iterate from highest to lowest probability until finding
    a token that is not a special token and not a newline character.
    ...
    Returns:
        decoded_sentences: list of decoded strings
    """
    ...
    for batch_idx in range(batch_size):
        ...
        for pos in range(seq_len):
            ...
            # Find first token that is not special token and not newline
            for token_id in sorted_indices:
                ...
                # Decode to check if it contains a newline
